...
function nodespec(name, deps) {
  ...
  Object.defineProperty(func, 'formatters', {
    get: function() {
      ...
      formatters = [];
      formatters.push(this.available_formatters[this.default_formatter]);
      return formatters.map(function(f) { return require(f).Formatter; });
    },
...
nodespec.before = function before(options, block) {
  ...
  block.timeout = options.timeout || this.DEFAULT_HOOK_TIMEOUT;
  block.timeout_after = function timeout_after(time) {
    block.timeout = time;
  }
...
function registerBuiltinFormatters(nodespec) {
  nodespec.registerFormatter(
    'progress', './formatters/progress_formatter', 'default');
}
