...
function Nodespec(name, deps) {
  ...
  this.addFormatter = function(formatter) {
    var f = this.available_formatters[formatter];
...
function registerBuiltinFormatters(nodespec) {
  ...
  nodespec.registerFormatter(
    'doc', './formatters/documentation');
}

function initFormatters(nodespec, conf, emitter) {

  if (conf.formatter && conf.formatter in nodespec.available_formatters) {
    nodespec.addFormatter(conf.formatter);
  }

  var formatters = [];
  nodespec.formatters.forEach(function(f) {
    var formatter = f.init(conf, emitter);
    if (formatter) {
...
function buildConfig() {
  ...
  // Verbosity
  if (~argv.indexOf('-v') || ~argv.indexOf('--verbose')) {
    conf.verbose = true;
  }
  // Stop on failure
  if (~argv.indexOf('--halt')) {
    conf.halt = true;
  }
  // Filter specs to run
  var grep_index = argv.indexOf('--grep');
  if (~grep_index) {
    var grep = new RegExp(argv[grep_index + 1]);
  ...
  // Colour support
  if (~argv.indexOf('--colour') ||
      ~argv.indexOf('--color')) {
    conf.colour = true;
  }
  if (~argv.indexOf('--no-colour') ||
      ~argv.indexOf('--no-color')) {
    conf.colour = false;
  }
  // Formatters
  var f_index = argv.indexOf('-f');
  if (~f_index) {
    conf.formatter = argv[f_index + 1];
  }
  return conf;
}
...
function displayHelp() {
  console.log("Usage: node <spec-file.js> [options]\n\
\n\
Options:\n\
  -v, --verbose      Show full stack traces on failures\n\
\n\
  --colour           Force formatter colour detection on\n\
  --no-colour        Force formatter colour detection off\n\
\n\
  --halt             Stop execution after any failing test\n\
  --grep '<regex>'   Only execute specs matching '<regex>'\n\
\n\
  --cov '<regex>'    Enable code coverage generation for all files matching\n\
                     '<regex>' - output will be written to ./coverage in a\n\
                     format similar to a unified diff\
");
}
