...
ProgressFormatter.prototype.LINE_WIDTH = 80;

ProgressFormatter.init = function(emitter) {
  return new ProgressFormatter(emitter);
}
function ProgressFormatter(emitter) {
  ...
  // TODO: proper colour detection
  if (tty.isatty(this.output_stream)) {
    COLOUR_ENABLED = true;
  }
      ...
      this.locateError(example, error, function(err, location, line) {
        ...
        if (isVerbose()) {
          ...
          for (var l in stack_lines) {
            this.writeln('       ' + stack_lines[l], red);
          }
        }
        i += 1;
        next();
