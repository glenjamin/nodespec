...
function Example(description, options, block) {
  ...
  this.line_number = parseInt(calling_line.line);
...
/*
  ...
  Arguments:
    ...
    conf    - {object} optional options to modify execution behaviour
...
*/
Example.prototype.exec = function(conf, emitter, callback/*(err, result)*/) {
  // conf is optional
  ...
  async.series(
    ...
    function(err) {
      example.exec_after(ctx, function(after_err) {
        ...
        // Allow tests to do stuff with unexpected errors
        var on_error = ctx.onError();
        if (err && on_error) {
          var internal_error = (err instanceof example.deps.Pending ||
                                err instanceof assert.AssertionError)
          if (!internal_error) {
            try {
              on_error.call(ctx, err);
              err = null;
            } catch (ex) {
              err = ex;
            }
