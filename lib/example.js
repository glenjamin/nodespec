...
Example.prototype.exec = function(exec_callback) {
    // No block, bail out early
    if (this.is_pending) {
        exec_callback(null, new SingleResult(Result.PEND));
        return;
    }
    var example = this;
    ...
    async.series(
        ...
        function(err, result) {
            if (err)
                handle_error(err, exec_callback)
            else
                exec_callback(null, new SingleResult(Result.PASS));
        }
    );
}
...
function handle_error(ex, callback) {
    ...
    if (ex instanceof Pending) {
        result = new SingleResult(Result.PEND);
    } else if (ex instanceof AssertionError) {
        result = new SingleResult(Result.FAIL);
    } else {
        result = new SingleResult(Result.ERROR);
    }
