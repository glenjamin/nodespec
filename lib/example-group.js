...
function ExampleGroup(description, options, definition_fn) {
  ...
  var it = function it(description, options, block) {
    group.example(description, options, block);
  };
  definition_fn.call(this, it);
}
...
ExampleGroup.prototype.exec = function(emitter, callback/*(err, result)*/) {
  ...
  // TODO: replace with proper filter
  ...
  if (~grep_index) {
    ...
    children = children.filter(function(e) {
      return (e instanceof ExampleGroup) || grep.test(e.full_description)
    })
  }
  ...
  async.forEachSeries(
    ...
    function iterator(child, next) {
      child.exec(emitter, function(err, result) {
        ...
        if (~process.argv.indexOf('--halt-on-fail') && result.error) {
          return next('halt');
        }
